...
Yii2UCenter is a collection of Yii2 components(widgets) based on responsive and multipurpose admin theme
called UCenter (v2.0). Powered with Twitter Bootstrap 3.1.0 Framework. 

UCenter can be used for any type of web applications: custom admin panels, admin dashboards, CMS, CRM, SAAS and websites: business, corporate, portfolio, blog.
UCenter has a sleek, clean and intuitive metro style design which makes your next project look awesome and yet user
friendly. UCenter has a huge collection of plugins and UI components and works seamlessly on all major web browsers,
tablets and phones. 

More than 30 widgets for Yii2 UCenter.

Basic configuration:
```
    'components' => [ 
        'metronic' => [
            'class' => 'hustshenl\metronic\UCenter',
            'color' => 'default',
            'layoutOption' => \hustshenl\metronic\UCenter::LAYOUT_FLUID,
            'headerOption' => 'fixed',
        ],
    ],
    'preload' => ['metronic'],
...
Some examples:
#### Menu
UCenter menu displays a multi-level menu using nested HTML lists.
...
Spinner renders an spinner Fuel UX widget.
 ...
 ```
#### Accordion 
   Accordion renders an accordion UCenter component.
 For example:

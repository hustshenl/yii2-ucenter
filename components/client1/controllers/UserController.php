...
use hustshenl\ucenter\components\client\models\ProtectedMembers;
use hustshenl\ucenter\components\client\models\User;
...
class UserController extends Component
{
    ...
    {
        $user = User::find()->where(['username'=>$args['username']])->one();
        if(empty($user->username)) {
            return -1;
        } elseif($user->password != md5(md5($args['password']).$user->salt)) {
            return -2;
        }
        return $user['uid'];
        var_dump($user);
    ...
    public function login($args) {
        ...
        if($args['isuid'] == 1) {
            $user = User::find()->where(['uid'=>$args['username']])->one();
        } elseif($args['isuid'] == 2) {
            $user = User::find()->where(['email'=>$args['username']])->one();
        } else {
            $user = User::find()->where(['username'=>$args['username']])->one();
        }
        if(empty($user)) {
            $status = -1;
            $user = new User();
        } elseif($user->validatePassword($args['password'])) {
            $status = -2;
        } elseif($args['checkques'] && $user->validateSecques($args['questionid'], $args['answer'])) {
            $status = -3;
        } else {
            $status = $user['uid'];
        }
    ...
    function register($args) {
        $args['regip'] = Yii::$app->request->userIP;

        $user = new User();
        if($user->load($args,'') === false) return false;
        if(($status = $user->validateUsername()) < 0) {
            return $status;
        }
        if(($status = $user->validateEmail()) < 0) {
            return $status;
        }
        if($user->save()===false){
    ...
    function edit($args) {
        ...
        if(!$ignoreoldpw && $args['email'] && ($status = User::check_emailexists($args['email'], $args['username'])) < 0) {
            return $status;
        }
        $user = User::findOne(['username' => $args['username']]);
        if($ignoreoldpw){
            $protectedMembers = ProtectedMembers::findOne(['username' => $args['username']]);
            if($protectedMembers) return -8;
        }
        if(!$ignoreoldpw&& !$user->validatePassword($oldpw)){
            return -1;
            /*$status = $user->validatePassword($oldpw);

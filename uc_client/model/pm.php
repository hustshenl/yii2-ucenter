...
class pmmodel {
	...
	function sendpm($fromuid, $fromusername, $touids, $subject, $message, $type = 0) {
		...
		$badwords = $this->base->cache('badwords');
		if($badwords['findpattern']) {
			$subject = @preg_replace($badwords['findpattern'], $badwords['replace'], $subject);
			$message = @preg_replace($badwords['findpattern'], $badwords['replace'], $message);
		}
		if(!$subject) {
